...
class BaseModel(object):
  ...
  """
    ...
    def init_embeddings(self, hparams, scope):
        """Init embeddings."""
        self._embedding_src, self._embedding_tgt = (
    ...
    """
        ...
        with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
            # Encoder
            encoder_outputs_src, encoder_state_src = self._build_encoder(hparams, self._embedding_src)
            encoder_outputs_tgt, encoder_state_tgt = self._build_encoder(hparams, self._embedding_tgt)

            loss_D_src = self._build_discriminator(encoder_outputs_src, tf.zeros([self.batch_size]))
            loss_D_tgt = self._build_discriminator(encoder_outputs_tgt, tf.ones([self.batch_size]))

            ## Decoder
            logits_s2s, sample_id_s2s, final_context_state_s2s = \
                self._build_decoder(encoder_outputs_src, encoder_state_src, hparams, self._embedding_src)

            logits_t2t, sample_id_t2t, final_context_state_t2t = \
                self._build_decoder(encoder_outputs_tgt, encoder_state_tgt, hparams, self._embedding_tgt)

            ## Loss
            if self.mode != tf.contrib.learn.ModeKeys.INFER:
                with tf.device(model_helper.get_device_str(num_layers - 1, num_gpus)):
                    loss = self._compute_loss(logits_s2s) + self._compute_loss(logits_t2t) #+ loss_D_src + loss_D_tgt
            else:
                loss = None

            return logits_s2s, loss, final_context_state_s2s, sample_id_s2s

    @abc.abstractmethod
    def _build_encoder(self, hparams, embedding_in) -> tuple:
    ...
    def _build_decoder(self, encoder_outputs, encoder_state, hparams, embedding_out):
            ...
            ## Inference
                ...
                if beam_width > 0:
                    ...
                    helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(
                        embedding_out, start_tokens, end_token)
    ...
    def _build_discriminator(self, encoder_outputs, labels):
        pass
